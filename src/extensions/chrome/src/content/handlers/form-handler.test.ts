...
// Mock login sender
jest.mock("../utils", () => ({
  sendLoginData: jest.fn(),
  monitorLoginResult: jest.fn(),
}));

// Mock detectors
jest.mock("../detectors", () => ({
  isUsernameField: jest.fn((input: HTMLInputElement) => {
    return (
      input.type === "email" ||
      input.name === "username" ||
  ...
  isPasswordField: jest.fn(
    (input: HTMLInputElement) => input.type === "password"
  ),
  isMFAField: jest.fn(() => false),
  handleSamlFormSubmit: jest.fn(),
}));
